...
class DailyBreadArchive extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, prayer, one_word', 'length', 'max'=>255),
			array('passage, key_verse', 'length', 'max'=>64),
			array('date, text, timestamp', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, date, title, passage, key_verse, text, prayer, one_word, timestamp', 'safe', 'on'=>'search'),
		);
	}
